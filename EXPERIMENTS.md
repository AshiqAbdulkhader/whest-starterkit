...
**Quasi-Monte Carlo (scrambled Sobol) — real, different-mechanism win.** `scripts/test_qmc.py`: replacing the pseudo-random Gaussian input with a scrambled Sobol sequence (inverse-CDF transformed), same total sample count, same FLOP cost, gives **1.30x** over pseudo-random MC (2.75e-6 vs 3.58e-6, 100 full-split MLPs) — a genuinely different variance-reduction axis (equidistribution of the *input*, not moment-fixing of an intermediate layer) that stacks with mmL1. Caveat: `scipy.stats.qmc.Sobol` needs `scipy`, which is **not** a declared/locked dependency (`pyproject.toml` only requires `flopscope`/`whestbench`) — unsafe to ship as-is since the grader may not have it. Testing whether pure-numpy Latin Hypercube Sampling (no external tables, no scipy) captures a similar share of the gain, since our own marginal-matching result suggests the win might be mostly per-dimension stratification rather than Sobol's joint low-discrepancy structure.
